-- L0A.2E.1 extraction RPC idempotency and retry-safe source dedup.

create table if not exists public.story_bible_extraction_requests (
...
alter table public.story_bible_extraction_requests enable row level security;

create unique index if not exists story_fact_sources_retry_dedup_idx
on public.story_fact_sources(
  project_id,
  extraction_run_id,
...
begin
  ...
  values (v_request_id, v_project_id, v_hash, v_run_id, 'running')
...
exception when others then
  update public.story_bible_extraction_requests
  set status = 'failed', error_code = sqlstate, completed_at = now()
  where request_id = v_request_id and status = 'running';
  raise;
end;
$$;
...
values ('p0_l0a2e_extraction_idempotency_dedup_013')

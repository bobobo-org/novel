...
echo.
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $d=Join-Path $env:TEMP ('NovelLocalAICompanion-'+[guid]::NewGuid().ToString('N')); New-Item -ItemType Directory -Path $d -Force ^| Out-Null; try { $s=Join-Path $d 'install.ps1'; $z=Join-Path $d 'companion.zip'; Invoke-WebRequest -UseBasicParsing -Uri 'https://novel-orcin.vercel.app/downloads/novel-local-ai-companion-install-v1.4.4.ps1' -OutFile $s; Invoke-WebRequest -UseBasicParsing -Uri 'https://novel-orcin.vercel.app/downloads/novel-local-ai-companion-v1.4.4.zip' -OutFile $z; if((Get-FileHash -LiteralPath $s -Algorithm SHA256).Hash -ne 'D54876BB8B55ABEAD8BEB269C82F8356B3250B0C58A3271A0C8C9B55ED9AB025'){throw 'INSTALL_SCRIPT_DIGEST_MISMATCH'}; if((Get-FileHash -LiteralPath $z -Algorithm SHA256).Hash -ne 'C130C5D9246339065414BD36709313888FA091A5DB09CC06739B0866F49390D7'){throw 'COMPANION_ARCHIVE_DIGEST_MISMATCH'}; ^& powershell.exe -NoProfile -ExecutionPolicy Bypass -File $s -ArchivePath $z; exit $LASTEXITCODE } finally { if(Test-Path -LiteralPath $d){Remove-Item -LiteralPath $d -Recurse -Force} }"

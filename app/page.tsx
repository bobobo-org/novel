...
export default function Home() {
  return <main className="frontdoor" data-consumer-release={RELEASE_MANIFEST.consumerRelease} data-app-commit={RELEASE_MANIFEST.appCommit}>
    ...
    <section className="frontdoorEntries" aria-labelledby="entryTitle"><div className="sectionTitle"><span>六種開始方式</span><h2 id="entryTitle">選一件現在最想做的事</h2></div><div className="entryGrid">{entries.map(([title, description, href], index) => <Link className="entryCard" href={href} key={title}><span className="entryIndex">{String(index + 1).padStart(2,"0")}</span><h3>{title}</h3><p>{description}</p><span className="entryArrow" aria-hidden="true">→</span></Link>)}</div></section>
    <footer className="frontdoorFooter"><p>作品預設保存在目前裝置。AI 產出先進入候選稿，不會直接覆蓋正文。</p><Link href="/professional">專業工具</Link></footer>
  </main>;
}

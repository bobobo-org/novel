...
export default function ProfessionalClient() {
  ...
  const reload=async()=>{setStatus((value)=>({...value,api:"正在檢查",error:""}));try{const response=await fetch("/api/ai/health",{cache:"no-store"}),raw=await response.json();if(!response.ok)throw new Error(raw?.userMessage||`狀態碼 ${response.status}`);setStatus({api:"正常",database:raw.databaseStatus==="reachable"?"正常":"需要注意",model:raw.legacyCloudAnalysis?.model||raw.legacyCloudAnalysis?.modelId||"尚未啟動",analyzer:raw.analyzerVersion||raw.storyAnalyzerVersion||"尚未提供",error:"",raw})}catch(reason){setStatus({api:"載入失敗",database:"無法確認",model:"無法確認",analyzer:"無法確認",error:reason instanceof Error?reason.message:"未知錯誤",raw:null})}try{const snapshot=await discoverStudioClosedAI();setLocal(snapshot.status==="ollama_ready"?"本機 AI 已連線":snapshot.status==="browser_ready"?"瀏覽器閉端 AI 已連線":snapshot.status==="auth_required"?"需要本機授權":"尚未啟動")}catch{setLocal("尚未啟動")}try{const saved=JSON.parse(localStorage.getItem("novel_p12_studio_state")||"null");setBackups(Array.isArray(saved?.backups)?saved.backups.length:0)}catch{setBackups(0)}};

...
import { pollVideoGenerationJob } from "@/lib/novel-ai/media-extension/server/video-job-service";
...
export async function GET(
  ...
  try {
    ...
    const job = await pollVideoGenerationJob(jobId, serverVideoJobDependencies());
    return NextResponse.json(job, { headers: VIDEO_SAFE_RESPONSE_HEADERS });
  } catch (error) {

...
import { retryVideoGenerationJob } from "@/lib/novel-ai/media-extension/server/video-job-service";
import {
...
export async function POST(
  ...
  try {
    ...
    const body = await readExternalAIJsonBody(request);
    const { jobId } = await context.params;
    const job = await retryVideoGenerationJob(jobId, body, serverVideoJobDependencies());

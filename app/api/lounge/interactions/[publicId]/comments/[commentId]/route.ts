...
type RouteContext = { params: Promise<{ publicId: string; commentId: string }> };

export async function DELETE(request: Request, context: RouteContext) {
  const { publicId, commentId } = await context.params;
  return handlers.deleteComment(request, publicId, commentId);
}

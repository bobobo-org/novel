...
export async function POST(req: Request) {
  ...
  try {
    const { result, aiRun } = await timedRun("continuity_review", input.context.projectId, input.context.chapterId, input, () =>
      createNovelProvider().reviewContinuity(input.context, input.candidateText),
    );
    return Response.json({ review: result, aiRunId: aiRun.id });
  } catch (error) {

...
export async function POST(req: Request) {
  try {
    ...
    return jsonError(error instanceof Error ? error.message : "建立故事記憶候選失敗。", 400, "MEMORY_CANDIDATE_ERROR");
  }
}
...
export async function PATCH(req: Request) {
  try {
    ...
    return jsonError(error instanceof Error ? error.message : "確認故事記憶失敗，原作品資料不會被覆蓋。", 400, "MEMORY_CONFIRM_ERROR");
  }
}

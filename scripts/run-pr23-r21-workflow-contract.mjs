...
assert.match(
  previewJob,
  /github\.event_name == 'pull_request'.*github\.event_name == 'workflow_dispatch' && inputs\.operation == 'deploy-preview'/u,
);
assert.match(globalConfiguration, /- deploy-preview/u);
assert.match(validateJob, /inputs\.operation == 'deploy-preview'/u);
for (const secret of [
  "VERCEL_TOKEN",
  "VERCEL_ORG_ID",
  "VERCEL_PROJECT_ID",
...
assert.match(previewJob, /ref:\s*\$\{\{ github\.event_name == 'pull_request' && github\.event\.pull_request\.head\.sha \|\| github\.sha \}\}/u);
assert.match(previewJob, /VERCEL_GIT_COMMIT_SHA:\s*\$\{\{ github\.event_name == 'pull_request' && github\.event\.pull_request\.head\.sha \|\| github\.sha \}\}/u);
...
assert.match(deployJob, /openai_safe/u);
assert.match(deployJob, /grok_safe/u);

...
await test("source-truth", async () => {
  ...
  assert.match(composer, /blocked:\s*externalBlocked,/u);
  assert.doesNotMatch(composer, /blocked:\s*externalBlocked \|\| closedAiStarting/u);
  assert.match(workspace, /const closedAiRuntimeRequired = plan\.executionKind === "closed_agent"[\s\S]*plan\.executionKind === "rpg"[\s\S]*plan\.executionKind === "attachment"/u);
  assert.match(workspace, /closedAiStartupStateRef\.current = closedAiStartupState/u);
  assert.match(workspace, /!externalSelectedForRequest[\s\S]*closedAiStartupStateRef\.current === "starting"[\s\S]*closedAiRuntimeRequired/u);
  assert.match(workspace, /CONVERSATION_CLOSED_AI_STARTING/u);
  assert.doesNotMatch(workspace, /closedAiRuntimeRequired[\s\S]{0,240}plan\.executionKind === "(?:query|repository|learning_import)"/u);
  assert.match(workspace, /retryActionRef\.current = \(\) => \{[\s\S]*initialPromptSenderRef\.current\([\s\S]*retryAfterClosedAiStartup/u);
  assert.doesNotMatch(workspace, /retryActionRef\.current = \(\) => \{ void sendRequest\(content/u);
  assert.match(workspace, /operationLockRef\.current = true;\s*\n\s*if \(retryAfterClosedAiStartup\) initialPromptUsed\.current = true/u);
  assert.match(workspace, /if \(retryAfterClosedAiStartup\) initialPromptUsed\.current = false/u);
  assert.match(workspace, /\}, undefined, initialTaskType, true\)\.finally/u);
  assert.match(workspace, /if \(operationLockRef\.current\) \{\s*\n\s*schedule\(50\);/u);
  assert.match(workspace, /!initialPromptUsed\.current[\s\S]*closedAiStartupStateRef\.current !== "starting"[\s\S]*schedule\(50\)/u);
  assert.match(workspace, /\[activeSession, busy, closedAiStartupState, initialPrompt, initialTaskType, loading\]/u);

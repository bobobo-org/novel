...
assert.match(chatRpgController, /import\("@\/lib\/novel-ai\/web\/rpg-chat-turn"\)/u);
assert.match(chatRpgController, /const fallbackPlan = await runtime\.buildRpgRuleChoicePlan\(\{/u);
...
assert.match(chatRpgController, /runtime\.planRpgChatChoices\(\{/u);
assert.match(chatRpgController, /fallbackReason: "USER_REQUESTED_RULE_FALLBACK"/u);
assert.match(chatRpgController, /最長等待 180 秒/u);
assert.doesNotMatch(chatRpgController, /RPG_CHOICE_RULE_PLAN_IMMEDIATE/u);
...
assert.match(rpgTurn, /export const RPG_CHAT_CHOICE_AI_TIMEOUT_MS = 180_000/u);
assert.match(rpgTurn, /enhancementController\.abort\("RPG_CHOICE_AI_ENHANCEMENT_TIMEOUT"\)/u);
assert.match(rpgApproval, /import\("@\/lib\/novel-ai\/web\/rpg-chat-turn"\)/u);
assert.match(rpgApproval, /await rpgRuntime\.approveRpgChatTurn\(\{/u);

...
await test("06 Stale revision", async () => { const row = await fixture("stale"); await assert.rejects(() => row.repository.acceptChoiceTransaction({ ...row.input, expectedProjectRevision: row.project.revision - 1 }), /PROJECT_REVISION_CONFLICT/); const bible = (await row.repository.list("storyBibles",row.project.id))[0]; await row.repository.put("storyBibles", bible, bible.revision); await assert.rejects(() => row.repository.acceptChoiceTransaction(row.input), (error) => error?.code === "STORY_BIBLE_REVISION_CONFLICT"); const missingBibleRevision = { ...row.input }; delete missingBibleRevision.expectedStoryBibleRevision; await assert.rejects(() => row.repository.acceptChoiceTransaction(missingBibleRevision), (error) => error?.code === "STORY_BIBLE_REVISION_CONFLICT"); assert.equal((await row.repository.list("approvalTransactions", row.project.id)).length, 0); });
...
await test("24 Corrupt and incomplete backup rejection", async () => { assert.equal((await validateBackupPayload(null)).reason, "BACKUP_INVALID_FORMAT"); const partial = structuredClone(backupPayload); for (const store of ["acceptedChoices", "storyBranches", "storyBibleDeltas", "approvalTransactions", "idempotencyRecords"]) delete partial.records[store]; partial.manifest.includedStores = Object.keys(partial.records); partial.manifest.recordCounts = Object.fromEntries(Object.entries(partial.records).map(([store, rows]) => [store, rows.length])); partial.manifest.contentHash = hash(partial.records); assert.equal((await validateBackupPayload(partial)).reason, "BACKUP_REQUIRED_STORE_MISSING"); });
...
await test("26 Schema mismatch and partial replace rejection", async () => { const broken = structuredClone(backupPayload); broken.manifest.projectSchemaVersion = "future-v99"; assert.equal((await validateBackupPayload(broken)).reason, "BACKUP_SCHEMA_UNSUPPORTED"); const row = await fixture("partial-replace"); const partial = structuredClone(backupPayload.records); for (const store of ["acceptedChoices", "storyBranches", "storyBibleDeltas", "approvalTransactions", "idempotencyRecords"]) delete partial[store]; await assert.rejects(() => row.repository.importProject(partial, "replace", row.project.id), /BACKUP_REQUIRED_STORE_MISSING/); assert.equal((await row.repository.listAcceptedChoices(row.project.id)).length, 0); });
...
await test("37 Professional tools health", async () => { const source = await readFile(new URL("../app/professional/professional-client.tsx", import.meta.url), "utf8"); assert.match(source, /系統狀態|本機 AI|本機AI/); assert.match(source, /作品管理中心/); assert.match(source, /研究與作者輔助/); assert.doesNotMatch(source, /href="\/legacy\/novel-system\.html/); });

...
function companionZipGate() {
  const zipRelative = "public/downloads/novel-local-ai-companion-v1.0.0.zip";
  const zip = fs.readFileSync(path.join(root, zipRelative));
  const checksumLine = fs.readFileSync(
    path.join(root, "public/downloads/novel-local-ai-companion-v1.0.0.sha256"),
    "ascii",
  ).trim();
  ...
  const prefix = "novel-local-ai-companion-v1.0.0/";

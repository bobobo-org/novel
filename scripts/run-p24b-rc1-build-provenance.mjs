...
test("sealed appCommit is exactly RC1 Product", () => assert.equal(provenance.appCommit, productCommit));
test("sealed releaseTag is P2.4B RC1", () => assert.equal(provenance.releaseTag, manifest.releaseTag));
test("sealed architectureStage is P2.4B RC", () => assert.equal(provenance.architectureStage, manifest.architectureStage));
test("sealed provenance source is allowed", () => assert.ok(contract.allowedProvenanceSources.includes(provenance.source)));
test("Legacy build appCommit is exactly RC1 Product", () => assert.equal(legacyBuild.commit, productCommit));
test("Legacy build identity matches complete RC1 manifest", () => {
...
test("Legacy static HTML contains exact RC1 Product identity", () => {
  for (const value of [
    productCommit,
    manifest.releaseTag,
...
test("Legacy static JavaScript contains exact RC1 Product identity", () => {
  for (const value of [
    productCommit,
    manifest.releaseTag,
    manifest.releaseName,
...
test("sealed outputs contain no evidence or CI commit substitution", () => {
  const forbidden = [
    process.env.P24B_RC1_EVIDENCE_COMMIT,
    process.env.P24B_RC1_CI_HEAD,
  ].filter(Boolean);
...
const report = {
  schemaVersion: "p24b-rc1-build-provenance-results-v1",
...
fs.writeFileSync(
  path.join(evidenceDir, "build-provenance-results.json"),
  `${JSON.stringify(report, null, 2)}\n`,
  "utf8",
);
console.log(JSON.stringify({ status: report.status, pass, fail, skip: 0, appCommit: report.appCommit }));

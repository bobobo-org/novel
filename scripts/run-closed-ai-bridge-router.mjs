...
import { LocalBridgeClient, configureLocalBridgeClient, configureLocalBridgeModel, selectAvailableTextModel } from "../lib/novel-ai/providers/local-ollama/local-bridge-client.ts";
...
import { containsConvertibleSimplifiedChinese } from "../lib/novel-ai/language/traditional-chinese.ts";
...
try {
  ...
  const modelId = selectAvailableTextModel(models.models, "qwen2.5:3b");
  assert.ok(modelId);
  await client.verifyModel(modelId);
  ...
  assert.equal(containsConvertibleSimplifiedChinese(result.content), false);
...
} finally { configureLocalBridgeClient(null); configureLocalBridgeModel(null); await bridge.stop(); }

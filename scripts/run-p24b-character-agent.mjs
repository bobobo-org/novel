...
function registerSecurityTests() {
  ...
  test("security", "Capability Truth keeps training and distillation not started", () => { assert.equal(CAPABILITY_TRUTH_MATRIX.find((item) => item.id === "modelTraining")?.status, "not_started"); assert.equal(CAPABILITY_TRUTH_MATRIX.find((item) => item.id === "distillation")?.status, "not_started"); });
}

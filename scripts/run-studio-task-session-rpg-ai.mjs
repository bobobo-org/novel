...
await test("source contracts expose save-home-task gating and verified closed AI RPG execution", async () => {
  ...
  assert.match(chatRpg, /import\("@\/lib\/novel-ai\/web\/rpg-chat-turn"\)/u);
  assert.match(chatRpg, /plan = await runtime\.planRpgChatChoices\(\{/u);
  assert.match(chatRpg, /const fallbackPlan = await runtime\.buildRpgRuleChoicePlan\(\{/u);
  ...
  assert.match(chatRpg, /rpgRuntime\.generateRpgChatTurnCandidate\(/u);

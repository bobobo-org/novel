...
const output = path.resolve(args.output || "deny-restart-verification.json");
...
try {
  ...
  const result = {
    schemaVersion: "r1k-browser-deny-restart-verification-v1", capturedAt: new Date().toISOString(), browser, profile, targetUrl,
    beforeReload, afterReload,
    loopbackRequests: loopback.filter((row) => row.phase === "request").length,
    loopbackResponses: loopback.filter((row) => row.phase === "response").length,
    externalAiCalls: network.filter((row) => /(?:api\.openai\.com|generativelanguage\.googleapis\.com|api\.x\.ai)/i.test(row.url)).length,
    status: beforeReload.denied && afterReload.denied && loopback.filter((row) => row.phase === "response").length === 0 ? "PASS" : "FAIL",
  };
...
async function snapshot(page) {
  const permissions = await page.evaluate(async () => {
    ...
    for (const name of ["loopback-network", "local-network-access"]) {
      try { result[name] = (await navigator.permissions.query({ name })).state; } catch { result[name] = "unsupported"; }
    }
    return result;
  });
  ...
  return {
    permissions,
    denied: permissions["loopback-network"] === "denied" || permissions["local-network-access"] === "denied",
    denialMessageVisible: text.includes("拒絕") && text.includes("本機網路"),
    noExternalFallbackMessageVisible: text.includes("不會改用外部 AI"),
  };
}

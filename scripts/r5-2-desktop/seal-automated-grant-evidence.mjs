...
const [sourceRootArg, runId, bundleRootArg, productCommit, harnessCommit, contractCommit] = process.argv.slice(2);
if (![sourceRootArg, runId, bundleRootArg, productCommit, harnessCommit, contractCommit].every(Boolean)) {
  throw new Error("sourceRoot, runId, bundleRoot, productCommit, harnessCommit, contractCommit are required.");
}
...
const checks = [
  ...
  ["formalChromeIdentity", final.identity?.fileProduct === "Google Chrome"],
  ...
  ["semanticAllowDecision", ["允許", "Allow"].includes(decision.elementName)],
  ["fixedCoordinatesNotUsed", decision.fixedScreenCoordinatesUsed === false],
  ["permissionGranted", final.permissionStates?.["loopback-network"] === "granted"],
  ["originAuthorizedVisible", final.uiTextAfterDecision.includes("目前網站已授權")],
  ["bridgeReachableVisible", final.uiTextAfterDecision.includes("本機橋接服務已啟動")],
...
const evidence = {
  schemaVersion: "r1k-chrome-grant-final-evidence-v1",
  verdict: "R1K_CHROME_GRANT_AUTOMATED_PASS",
  ...
  productCommit,
  harnessCommit,
  evidenceContractCommit: contractCommit,
  runId,
  previewUrl: final.preview.url,
  native_prompt: {
    chrome_pid: decision.processId,
    window_title: decision.mainWindowTitle,
    automation_role: decision.automationRole,
    clicked_control_name: decision.elementName,
    selection_method: "SEMANTIC_UI_AUTOMATION_NAME_ROLE_AND_PID",
    fixed_screen_coordinates_used: false,
  },
  permission: {
    before: "ASK_OR_UNSET",
    after: "GRANTED",
    setting: 1,
    browser_states: final.permissionStates,
    reload_states: restart.permissionAfterReload,
    chrome_restart_persisted: restart.permissionPersisted,
  },
  bridge: {
    reachable: final.uiTextAfterDecision.includes("本機橋接服務已啟動"),
    origin_authorized: final.uiTextAfterDecision.includes("目前網站已授權"),
    paired: pairing.status === "PAIRED",
    pairing_interaction: pairing.interaction,
    pairing_code_persisted: pairing.pairingCodePersisted,
    browser_request_count: loopbackRequests.length,
    browser_response_count: loopbackResponses.length,
    stale_pairing_reused_after_restart: restart.stalePairingReused,
  },
  repository: {
    formal_approval_transaction_included: false,
    formal_mutation_count: 0,
    accepted_choices_mutation_count: 0,
    story_branches_mutation_count: 0,
    story_bible_mutation_count: 0,
    mutation_audit_status: mutation.status,
  },
  privacy: { external_ai_calls: externalAiCalls.length, direct_api_substitution: false },
  security: {
    forbidden_arguments: final.securityAudit.forbiddenArgumentCount,
...
const evidenceName = "r1k-chrome-grant-final-evidence.json";
await writeFile(path.join(bundleRoot, evidenceName), `${JSON.stringify(evidence, null, 2)}\n`, "utf8");
const copies = [
  [path.join(sourceRoot, "automated-grant-preflight.json"), "automated-grant-preflight.json"],
  ...
  [path.join(runRoot, "native-lna-after-grant.png"), "native-lna-after-grant.png"],
  [path.join(runRoot, "preview-before.png"), "preview-before.png"],
  [path.join(runRoot, "preview-after.png"), "preview-after.png"],
  [path.join(runRoot, "final-result.json"), "final-result.json"],
  [path.join(runRoot, "network.json"), "network.json"],
  [path.join(runRoot, "console.json"), "console.json"],
  [path.join(runRoot, "repository-mutation-audit.json"), "repository-mutation-audit.json"],
  [path.join(runRoot, "storage-before.json"), "storage-before.json"],
  [path.join(runRoot, "storage-after.json"), "storage-after.json"],
  [path.join(runRoot, "cleanup.json"), "adapter-cleanup.json"],
  [path.join(runRoot, "browser-trace.zip"), "browser-trace.zip"],
];
...
const manifest = {
  schemaVersion: "r1k-chrome-grant-immutable-manifest-v1",
  createdAt: new Date().toISOString(), productCommit, harnessCommit, evidenceContractCommit: contractCommit, runId, records,
};
...
const seal = {
  schemaVersion: "r1k-chrome-grant-bundle-seal-v1",
  sealedAt: new Date().toISOString(),
  manifestSha256: await sha256(path.join(bundleRoot, "evidence-manifest.json")),
  checksumsSha256: await sha256(path.join(bundleRoot, "checksums.sha256")),

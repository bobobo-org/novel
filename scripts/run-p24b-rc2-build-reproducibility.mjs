...
const expectedTag = "novel-ai-p24b-character-agent-rc2";
...
const TEXT_BUILD_OUTPUT = /\.(?:html|js|json|map|rsc|txt)$/;

function normalizeBuildRecord(name, buildId) {
  const bytes = fs.readFileSync(path.join(root, name));
  const semanticPath = name.replace(
    `.next/static/${buildId}/`,
    ".next/static/__NEXT_BUILD_ID__/",
  );
  const text = TEXT_BUILD_OUTPUT.test(name) ? bytes.toString("utf8") : null;
  const normalizedText = text?.replaceAll(buildId, "__NEXT_BUILD_ID__") ?? null;
  const normalizedBytes = normalizedText === null ? bytes : Buffer.from(normalizedText, "utf8");
  return {
    path: semanticPath,
    sourcePath: name,
    ...
    nextBuildIdOccurrencesNormalized: text === null
      ? 0
      : text.split(buildId).length - 1,
  };
}
...
function releaseRelevantPaths() {
  ...
  const dynamic = listFiles(path.join(root, ".next"))
    .map((name) => `.next/${name}`)
    .filter((name) =>
      name.startsWith(".next/static/")
      || /^\.next\/server\/app\/api\/(?:ai\/health|admin\/persistence)\//.test(name)
      || /^\.next\/server\/app\/(?:page|studio|professional)\//.test(name)
...
function runBuild(buildNumber) {
  ...
  execFileSync(command, args, {
    ...
    env: {
      ...
      NOVEL_BUILD_SEALED_AT: sealedAt,
    },
    stdio: "inherit",
  });
  const provenance = JSON.parse(
    fs.readFileSync(path.join(root, "generated/release-provenance.json"), "utf8"),
  );
  const legacy = JSON.parse(
    fs.readFileSync(path.join(root, "public/legacy/novel-system.build.json"), "utf8"),
  );
  ...
  const records = releaseRelevantPaths().map((name) => normalizeBuildRecord(name, buildId));
...
const missing = first.records
  .filter((record) => !secondByPath.has(record.path))
  .map((record) => record.path);
const unexpected = second.records
  .filter((record) => !firstByPath.has(record.path))
  .map((record) => record.path);
const mismatch = second.records
  .filter((record) => {
...
const status = missing.length === 0 && unexpected.length === 0 && mismatch.length === 0
  ...
  : "FAIL";
const report = {
  schemaVersion: "p24b-rc2-build-reproducibility-v1",
  generatedAt: new Date().toISOString(),
  productCommit,
  releaseTag: expectedTag,
  architectureStage: expectedStage,
  buildCount: 2,
  discoveryMode: "dynamic-release-relevant-artifact-manifest-with-next-build-id-normalization",
  normalizedVolatileFields: ["nextBuildId"],
  first,
  second,
  missing,
  unexpected,
...
fs.writeFileSync(
  path.join(evidenceDir, "build-reproducibility.json"),

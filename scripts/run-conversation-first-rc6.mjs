...
function storyWithParagraphs(paragraphCount, charactersPerParagraph) {
  const actors = ["明檀", "柳澄", "裴照", "沈硯", "溫荷", "程野", "蘇衡", "季蘭", "白朔", "顧遙", "謝棠", "林岫", "周晏", "江琢", "葉霜", "唐岑", "陸遲"];
  const places = ["西廊窗下", "封存庫門前", "雨棚盡頭", "鐘樓石階", "藥房後院", "河埠木棧", "議事廳側門", "荒園井邊", "城牆暗梯", "客棧灶間", "渡口倉房", "書院竹徑", "舊戲臺後", "巡夜值房", "染坊水渠", "山門碑亭", "廢塔頂層"];
  const discoveries = ["一枚沾著新泥的銅扣", "被火燎過半角的名冊", "刻錯時辰的巡守木牌", "藏在茶磚裡的短箋", "尚帶體溫的藥囊", "逆著水流漂回的燈罩", "少了一枚印記的契書", "井繩上新換的麻結", "箭孔旁殘留的藍蠟", "灶灰下壓著的銀票", "木箱夾層中的船籍", "竹葉背面的炭筆字", "戲服袖口縫入的鑰匙", "值簿中被刮去的人名", "染缸底部沉著的徽章", "碑座後方新刻的箭頭", "齒輪縫裡卡住的髮簪"];
  const actions = ["她先封住窗縫，再讓送信人說出暗號", "他沒有碰名冊，只叫見證者逐頁念出缺口", "眾人分開查驗腳印與木牌，誰也不能替誰作證", "她把短箋交給最不可能合作的人，逼暗處的人改變計畫", "他循著藥味追到院牆，卻先救下倒在雨裡的學徒", "她熄掉引路燈，改用水面倒影辨認尾隨者", "他當眾重讀契書，讓兩派都承認同一條期限", "她割下一截井繩保存，隨即沿濕痕追向後巷", "他以藍蠟比對三處箭孔，確認襲擊者曾進過內城", "她翻開灶灰前先叫掌櫃封門，免得證物再被調換", "他查對船籍與潮汐，找出一艘從未靠岸的貨船", "她把炭筆字拓下來，沒有驚動躲在竹林裡的眼線", "他換上戲服登臺，以錯誤口令引出真正持鑰人", "她讓兩名巡夜人分室覆述，終於聽出被刮掉的時辰", "他排乾水渠取回徽章，也暴露自己知道舊案", "她順著箭頭走到碑亭外，留下同伴守住唯一退路", "他停住塔鐘，讓齒輪裡的髮簪成為無法否認的證據"];
  const consequences = ["廊外腳步因此停了半拍，藏在雨聲裡的人知道自己已被識破。", "封存庫的燈一盞盞亮起，原先沉默的守衛終於願意指出昨夜換班者。", "雨棚下的爭執被迫停住，兩份彼此矛盾的口供第一次有了可核對的時間。", "鐘聲落下以前，對方送來新的交換條件，也承認短箋確實碰到要害。", "院牆後傳來車輪離去的聲音，救人與追查從此成為同一筆代價。", "黑船失去燈號後偏離航道，河面卻浮出另一串更近的波紋。", "側門被內應鎖死，議事廳裡的人只剩公開表態或當場離席兩條路。", "後巷盡頭有人丟下染血手套，證明失蹤者離開時仍在反抗。", "城牆守軍開始換哨，主角必須趁命令尚未傳遍前決定是否越界。", "掌櫃終於交出帳簿，但要求眾人保住一個無辜夥計的名字。", "潮水退去時，貨船的纜痕指向官署私用的碼頭。", "竹林裡傳來折枝聲，眼線選擇撤退，也留下能被追蹤的布片。", "臺下喝采掩住短促驚呼，持鑰人已在眾目睽睽下露出破綻。", "兩份覆述只差一刻鐘，那一刻鐘正好容得下有人改寫值簿。", "舊案倖存者看見徽章後拒絕再逃，願意帶眾人去找被封的染房。", "碑亭後的石門緩慢移開，退路卻同時被趕來的追兵截斷。", "停住的塔鐘驚動全城，證據保住了，潛伏者也知道調查已逼近核心。"];
  return Array.from({ length: paragraphCount }, (_, index) => {
    const slot = index % actors.length;
    const paragraph = `第${index + 1}段，${places[slot]}傳來異響。${actors[slot]}發現${discoveries[slot]}。${actions[slot]}。${consequences[slot]}`;
    const atmosphericTail = `雨勢第${index + 1}次轉急，遠近燈影沿著不同方向晃動，沒有人能再把眼前變化說成偶然。`;
    return `${paragraph}${atmosphericTail.repeat(Math.max(0, Math.ceil((charactersPerParagraph - paragraph.length) / atmosphericTail.length)))}`;
  }).join("\n\n");
}

...
type BrowserExactOversizedStopStageEvidence = {
  contractSatisfied: boolean;
  safetyCode: string | null;
  failureCode: string | null;
  rawBudgetExceeded: boolean;
...
function isBrowserExactOversizedStopStageEvidence(
  input: BrowserExactOversizedStopStageEvidence,
  requiredStageHardCap: number,
) {
  return isBrowserOversizedStopIsolatedRepairCandidate({
    contractSatisfied: input.contractSatisfied,
    safetyCode: input.safetyCode,
    failureCode: input.failureCode,
    rawBudgetExceeded: input.rawBudgetExceeded,
    observedHanCharacters: input.observedHanCharacters,
    observedEstimatedTokens: input.observedEstimatedTokens,
    observedCodePoints: input.observedCodePoints,
    finishReason: input.finishReason,
    completionTokens: input.completionTokens,
    runtimeTokenCap: input.runtimeTokenCap,
    stageHardCap: input.stageHardCap,
  })
    && Number.isSafeInteger(input.observedHanCharacters)
    && input.observedHanCharacters > BROWSER_PROSE_MAXIMUM_HAN_CHARACTERS
    && Number.isSafeInteger(input.observedEstimatedTokens)
    && input.observedEstimatedTokens >= 1
    && Number.isSafeInteger(input.observedCodePoints)
    && input.observedCodePoints >= input.observedHanCharacters
    && Number.isSafeInteger(input.runtimeTokenCap)
    && input.runtimeTokenCap >= 1
    && Number.isSafeInteger(input.stageHardCap)
    && input.stageHardCap === requiredStageHardCap
    && Number.isSafeInteger(input.contentCharacters)
    && input.contentCharacters >= input.observedCodePoints
    && input.contentCharacters <= input.observedCodePoints * 2
    && input.observedEstimatedTokens
      <= Math.ceil(input.contentCharacters * 1.08)
    && input.reportedOutputCharacters === input.contentCharacters
    && input.reportedRawOutputCharacters === input.contentCharacters
    && (input.normalizationDeferred
      ? input.reportedNormalizedOutputCharacters === undefined
      : input.reportedNormalizedOutputCharacters === input.contentCharacters);
}

export function isBrowserRepeatedOversizedStopFreshRecoveryCandidate(input: {
  initial: BrowserExactOversizedStopStageEvidence;
  repair: BrowserExactOversizedStopStageEvidence;
}) {
  return isBrowserExactOversizedStopStageEvidence(
    input.initial,
    BROWSER_PROSE_MAXIMUM_ESTIMATED_TOKENS,
  ) && isBrowserExactOversizedStopStageEvidence(
    input.repair,
...
type BrowserExactDiscardedStageEvidence = Omit<
  BrowserExactTruncatedInitialStageEvidence,
  "qualityReasonCodes"
> & {
...
export function isBrowserDegenerateInitialOversizedStopRepairFreshRecoveryCandidate(
  input: {
    initial: BrowserExactDiscardedStageEvidence;
    repair: BrowserExactDiscardedStageEvidence;
  },
) {
  const initial = input.initial;
  const repair = input.repair;
  const repairSelectionIsDiscardable = !repair.selectedContentAvailable
    && (
      (
        !repair.salvageableContentAvailable
        && repair.selectedHanCharacters === 0
      ...
      || (
        repair.salvageableContentAvailable
        && Number.isSafeInteger(repair.selectedHanCharacters)
        && repair.selectedHanCharacters >= BROWSER_PROSE_MINIMUM_HAN_CHARACTERS
        && repair.selectedHanCharacters <= BROWSER_PROSE_MAXIMUM_HAN_CHARACTERS
        && Number.isSafeInteger(repair.selectedEstimatedTokens)
        && repair.selectedEstimatedTokens >= 1
        && repair.selectedEstimatedTokens
          <= BROWSER_PROSE_MAXIMUM_ESTIMATED_TOKENS
        && Number.isSafeInteger(repair.selectedCodePoints)
        && repair.selectedCodePoints >= repair.selectedHanCharacters
        && repair.selectedCodePoints <= BROWSER_PROSE_MAXIMUM_CODE_POINTS
      )
    );
  return !initial.contractSatisfied
    && !initial.safetyCode
    && initial.failureCode === "minimum-length-unmet"
    && !initial.rawBudgetExceeded
    && Number.isSafeInteger(initial.observedHanCharacters)
    && initial.observedHanCharacters > 0
    && initial.observedHanCharacters
      < BROWSER_PROSE_MINIMUM_CONTINUATION_BASE_HAN_CHARACTERS
    && Number.isSafeInteger(initial.observedEstimatedTokens)
    && initial.observedEstimatedTokens >= 1
    && initial.observedEstimatedTokens <= 64
    && Number.isSafeInteger(initial.observedCodePoints)
    && initial.observedCodePoints >= initial.observedHanCharacters
    && initial.observedCodePoints <= 64
    && initial.selectedHanCharacters === 0
    && initial.selectedEstimatedTokens === 0
    && initial.selectedCodePoints === 0
    && !initial.selectedContentAvailable
    && !initial.salvageableContentAvailable
    && initial.finishReason === "stop"
    && Number.isSafeInteger(initial.completionTokens)
    && (initial.completionTokens ?? 0) >= 1
    && (initial.completionTokens ?? 0) <= 64
    && Number.isSafeInteger(initial.runtimeTokenCap)
    && initial.runtimeTokenCap === BROWSER_PROSE_MAXIMUM_ESTIMATED_TOKENS
    && Number.isSafeInteger(initial.stageHardCap)
    && initial.stageHardCap === BROWSER_PROSE_MAXIMUM_ESTIMATED_TOKENS
    ...
    && Number.isSafeInteger(initial.contentCharacters)
    && initial.contentCharacters >= initial.observedCodePoints
    && initial.contentCharacters <= 64
    && initial.contentCharacters <= initial.observedCodePoints * 2
    && initial.observedEstimatedTokens
      <= Math.ceil(initial.contentCharacters * 1.08)
    && initial.reportedOutputCharacters === initial.contentCharacters
    && initial.reportedRawOutputCharacters === initial.contentCharacters
    ...
    && repair.rawBudgetExceeded
    && Number.isSafeInteger(repair.observedHanCharacters)
    && repair.observedHanCharacters >= 480
    && repair.observedHanCharacters <= 560
    && Number.isSafeInteger(repair.observedEstimatedTokens)
    && repair.observedEstimatedTokens >= 513
    && repair.observedEstimatedTokens <= 640
    && Number.isSafeInteger(repair.observedCodePoints)
    && repair.observedCodePoints >= repair.observedHanCharacters
    && repair.observedCodePoints <= BROWSER_PROSE_MAXIMUM_CODE_POINTS
    && repairSelectionIsDiscardable
    && repair.finishReason === "stop"
    && Number.isSafeInteger(repair.completionTokens)
    && (repair.completionTokens ?? 0) >= 336
    && Number.isSafeInteger(repair.runtimeTokenCap)
    && repair.runtimeTokenCap === BROWSER_BOUNDED_REPAIR_MAX_TOKENS
    && Number.isSafeInteger(repair.stageHardCap)
    && repair.stageHardCap === BROWSER_BOUNDED_REPAIR_MAX_TOKENS
    ...
    && repair.observedCodePoints <= (repair.completionTokens ?? 0) * 4 + 128
    && Number.isSafeInteger(repair.contentCharacters)
    && repair.contentCharacters >= 513
    && repair.contentCharacters <= BROWSER_PROSE_MAXIMUM_CODE_POINTS
    && repair.contentCharacters >= repair.observedCodePoints
    && repair.contentCharacters <= repair.observedCodePoints * 2
    && repair.observedEstimatedTokens
      <= Math.ceil(repair.contentCharacters * 1.08)
    && repair.reportedOutputCharacters === repair.contentCharacters
    && repair.reportedRawOutputCharacters === repair.contentCharacters
...
export function isBrowserDegenerateInitialNearCapInternalEnvelopeRepairFreshRecoveryCandidate(
  input: {
    initial: BrowserExactDiscardedStageEvidence;
    repair: BrowserExactDiscardedStageEvidence;
  },
  ...
  return !initial.contractSatisfied
    ...
    && repair.failureCode === null
    && !repair.rawBudgetExceeded
    && Number.isSafeInteger(repair.observedHanCharacters)
    && repair.observedHanCharacters
      >= BROWSER_PROSE_MINIMUM_CONTINUATION_BASE_HAN_CHARACTERS
    && repair.observedHanCharacters < BROWSER_PROSE_MINIMUM_HAN_CHARACTERS
    && Number.isSafeInteger(repair.observedEstimatedTokens)
    && repair.observedEstimatedTokens >= 1
    && repair.observedEstimatedTokens <= BROWSER_PROSE_MAXIMUM_ESTIMATED_TOKENS
    && Number.isSafeInteger(repair.observedCodePoints)
    && repair.observedCodePoints >= repair.observedHanCharacters
    && Number.isSafeInteger(repair.completionTokens)
    && repair.observedCodePoints >= (repair.completionTokens ?? 0)
    && repair.observedCodePoints <= BROWSER_PROSE_MAXIMUM_CODE_POINTS
    && repair.selectedHanCharacters === 0
    && repair.selectedEstimatedTokens === 0
    && repair.selectedCodePoints === 0
    && !repair.selectedContentAvailable
    && !repair.salvageableContentAvailable
    && repair.finishReason === "length"
    && Number.isSafeInteger(repair.runtimeTokenCap)
    && repair.runtimeTokenCap === BROWSER_BOUNDED_REPAIR_MAX_TOKENS
    && Number.isSafeInteger(repair.stageHardCap)
    && repair.stageHardCap === BROWSER_BOUNDED_REPAIR_MAX_TOKENS
    && (repair.completionTokens ?? 0) >= BROWSER_BOUNDED_REPAIR_MAX_TOKENS - 8
    && (repair.completionTokens ?? 0) <= repair.runtimeTokenCap
    && repair.observedCodePoints <= (repair.completionTokens ?? 0) * 4 + 128
    && Number.isSafeInteger(repair.contentCharacters)
    && repair.contentCharacters >= repair.observedCodePoints
...
export function isBrowserNearTargetInitialNearCapOversizedRepairFreshRecoveryCandidate(
  input: {
    initial: BrowserExactDiscardedStageEvidence;
    repair: BrowserExactDiscardedStageEvidence;
  },
  ...
  return !initial.contractSatisfied
    ...
    && Number.isSafeInteger(initial.observedHanCharacters)
    && initial.observedHanCharacters
      >= BROWSER_PROSE_MINIMUM_HAN_CHARACTERS - 8
    && initial.observedHanCharacters < BROWSER_PROSE_MINIMUM_HAN_CHARACTERS
    && Number.isSafeInteger(initial.observedEstimatedTokens)
    && initial.observedEstimatedTokens >= 1
    && initial.observedEstimatedTokens <= BROWSER_PROSE_MAXIMUM_ESTIMATED_TOKENS
    && Number.isSafeInteger(initial.observedCodePoints)
    ...
    && Number.isSafeInteger(repair.observedHanCharacters)
    && repair.observedHanCharacters > BROWSER_PROSE_MAXIMUM_HAN_CHARACTERS
    && repair.observedHanCharacters <= 416
    && Number.isSafeInteger(repair.observedEstimatedTokens)
    && repair.observedEstimatedTokens > BROWSER_PROSE_MAXIMUM_ESTIMATED_TOKENS
    && repair.observedEstimatedTokens <= 512
    && Number.isSafeInteger(repair.observedCodePoints)
    && repair.observedCodePoints >= repair.observedHanCharacters
    && repair.observedCodePoints <= 704
    && repair.selectedHanCharacters === 0
    && repair.selectedEstimatedTokens === 0
    && repair.selectedCodePoints === 0
    && !repair.selectedContentAvailable
    ...
    && repair.observedCodePoints <= (repair.completionTokens ?? 0) * 4 + 128
    && Number.isSafeInteger(repair.contentCharacters)
    && repair.contentCharacters >= BROWSER_PROSE_MAXIMUM_CODE_POINTS + 1
    && repair.contentCharacters <= 704
    && repair.contentCharacters >= repair.observedCodePoints
    && repair.contentCharacters <= repair.observedCodePoints * 2
    && repair.observedEstimatedTokens
      <= Math.ceil(repair.contentCharacters * 1.08)
...
export function isBrowserOversizedInitialCollapsedRepairFreshRecoveryCandidate(
  ...
  const repairCompletionTokens = input.repairCompletionTokens;
  return isBrowserOversizedStopIsolatedRepairCandidate({
    contractSatisfied: input.initialContractSatisfied,
    safetyCode: input.initialSafetyCode,
    failureCode: input.initialFailureCode,
    rawBudgetExceeded: input.initialRawBudgetExceeded,
    observedHanCharacters: input.initialObservedHanCharacters,
    observedEstimatedTokens: input.initialObservedEstimatedTokens,
    observedCodePoints: input.initialObservedCodePoints,
    finishReason: input.initialFinishReason,
    completionTokens: initialCompletionTokens,
    runtimeTokenCap: input.initialRuntimeTokenCap,
    stageHardCap: input.initialStageHardCap,
  })
    && Number.isSafeInteger(input.initialObservedHanCharacters)
    && Number.isSafeInteger(input.initialObservedEstimatedTokens)
    && input.initialObservedEstimatedTokens >= 1
    && Number.isSafeInteger(input.initialObservedCodePoints)
    && input.initialObservedCodePoints >= input.initialObservedHanCharacters
    ...
    && Number.isSafeInteger(input.initialContentCharacters)
    && input.initialContentCharacters >= input.initialObservedCodePoints
    && input.initialContentCharacters <= input.initialObservedCodePoints * 2
    && input.initialReportedRawOutputCharacters === input.initialContentCharacters
    && input.initialReportedNormalizedOutputCharacters
      === input.initialContentCharacters
    && !input.repairContractSatisfied
    ...
    && input.repairFailureCode === "minimum-length-unmet"
    && !input.repairRawBudgetExceeded
    && Number.isSafeInteger(input.repairObservedHanCharacters)
    && input.repairObservedHanCharacters > 0
    && input.repairObservedHanCharacters
      < BROWSER_PROSE_MINIMUM_CONTINUATION_BASE_HAN_CHARACTERS
    && input.repairFinishReason === "stop"
    ...
    && Number.isSafeInteger(input.repairObservedEstimatedTokens)
    && input.repairObservedEstimatedTokens >= 1
    && input.repairObservedEstimatedTokens <= input.repairRuntimeTokenCap
    && Number.isSafeInteger(input.repairObservedCodePoints)
    && input.repairObservedCodePoints >= input.repairObservedHanCharacters
    && input.repairObservedCodePoints <= BROWSER_PROSE_MAXIMUM_CODE_POINTS
    && input.repairObservedCodePoints
    ...
    && Number.isSafeInteger(input.repairContentCharacters)
    && input.repairContentCharacters >= input.repairObservedCodePoints
    && input.repairContentCharacters <= input.repairObservedCodePoints * 2
    && input.repairReportedRawOutputCharacters === input.repairContentCharacters
    && input.repairReportedNormalizedOutputCharacters
      === input.repairContentCharacters;
}
...
export async function executeBrowserBoundedQualityPasses(input: {
  ...
  if (
    ...
    const stoppedRepairInternalEnvelopeRecovery = Boolean(
    ...
    const nearCapLengthInternalEnvelopeFreshRecoveryCandidate = Boolean(
      !initialIsolatedRepairReasonCode
      && defaultChapterProseContract
      && requiredGenerativeExecutor === "webllm-worker"
      && executionRequest.contextAttestation === "required"
      ...
      && isBrowserDegenerateInitialNearCapInternalEnvelopeRepairFreshRecoveryCandidate({
        initial: {
          ...
          selectedContentAvailable: Boolean(chapterProseContract.content),
          salvageableContentAvailable: Boolean(
        ...
        repair: {
          ...
          selectedContentAvailable: Boolean(repairCompletion.content),
          salvageableContentAvailable: Boolean(
    ...
    const repairInternalEnvelopeRecovery =
      stoppedRepairInternalEnvelopeRecovery
      || nearCapLengthInternalEnvelopeFreshRecoveryCandidate;
    ...
    const oversizedInitialCollapsedRepairFreshRecoveryCandidate = Boolean(
      initialStopBudgetRepairReasonCode === "QUALITY_OUTPUT_TRUNCATED"
      ...
      && isBrowserOversizedInitialCollapsedRepairFreshRecoveryCandidate({
        ...
        repairObservedEstimatedTokens:
          repairCompletion.observedEstimatedTokens,
    ...
    const repeatedOversizedStopFreshRecoveryCandidate = Boolean(
      initialStopBudgetRepairReasonCode === "QUALITY_OUTPUT_TRUNCATED"
      && !initialSafetyRepairReasonCode
      && defaultChapterProseContract
      && requiredGenerativeExecutor === "webllm-worker"
    ...
    const oversizedInitialFreshRecoveryCandidate =
      oversizedInitialCollapsedRepairFreshRecoveryCandidate
      || repeatedOversizedStopFreshRecoveryCandidate;
    if (
    ...
    const nearTargetInitialNearCapOversizedRepairFreshRecoveryCandidate = Boolean(
      defaultChapterProseContract
      && requiredGenerativeExecutor === "webllm-worker"
      && !repairInternalEnvelopeRecovery
      && !initialIsolatedRepairReasonCode
    ...
    const degenerateInitialOversizedStopRepairFreshRecoveryCandidate = Boolean(
      defaultChapterProseContract
      && requiredGenerativeExecutor === "webllm-worker"
      && !repairInternalEnvelopeRecovery
      && !initialIsolatedRepairReasonCode
    ...
    if (
      ...
      && !nearTargetInitialNearCapOversizedRepairFreshRecoveryCandidate
      && !degenerateInitialOversizedStopRepairFreshRecoveryCandidate
      && !oversizedInitialFreshRecoveryCandidate
    ) {
      const selectedCompletion = selectVerifiedBrowserLengthSafePrefix({

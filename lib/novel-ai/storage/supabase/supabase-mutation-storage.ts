...
function normalizeValue(entityType: EntityType, field: string, value: unknown) {
  if (field === "age") {
    ...
    if (!Number.isInteger(num) || num < 0 || num > 2000) throw new StoryBibleMutationError("INVALID_FIELD_VALUE", "age must be an integer between 0 and 2000.", 422, { fieldPath: field, retryable: false });
    return num;
  }
  if (field === "lifeStatus") {
    const val = String(value);
    if (!["alive", "dead", "missing", "unknown"].includes(val)) throw new StoryBibleMutationError("INVALID_FIELD_VALUE", "lifeStatus is not supported.", 422, { fieldPath: field, retryable: false });
    return val;
  }
  if (entityType === "foreshadowing" && field === "status") {
    const val = String(value);
    if (!["planted", "developing", "partially_paid", "paid", "abandoned"].includes(val)) throw new StoryBibleMutationError("INVALID_FIELD_VALUE", "foreshadowing status is not supported.", 422, { fieldPath: field, retryable: false });
    return val;
  }
  if (entityType === "open_thread" && field === "status") {
    const val = String(value);
    if (!["open", "developing", "resolved", "abandoned"].includes(val)) throw new StoryBibleMutationError("INVALID_FIELD_VALUE", "open thread status is not supported.", 422, { fieldPath: field, retryable: false });
    return val;
  }
...
function validateField(entityType: EntityType, fieldPath: string) {
  ...
  if (!adapterMap[entityType].fields.includes(field)) {
    throw new StoryBibleMutationError("FIELD_PATH_NOT_SUPPORTED", `Unsupported fieldPath: ${fieldPath}`, 422, { fieldPath, entityType, retryable: false });
  }
  return field;
}
...
function validateTransition(entityType: EntityType, field: string, previous: unknown, next: unknown, candidate: JsonRecord, sourceMode: SourceMode, conflicts: JsonRecord[]) {
  ...
  if (blocking.length > 0) {
    throw new StoryBibleMutationError("BLOCKING_CONFLICT_PRESENT", "Candidate has blocking conflicts and cannot be approved.", 409, {
      conflictIds: blocking.map((x) => x.id),
      retryable: false,
    });
  }
  if (sourceMode === "ai-supported" && candidate.source_valid === false) {
    throw new StoryBibleMutationError("INVALID_SOURCE_REFERENCE", "AI-supported approval requires a valid source reference.", 409, { retryable: false });
  }
  if (entityType === "world_rule" && field !== "immutable" && candidate.operation !== "create") {
    ...
    if ((candidate as JsonRecord).immutable === true) {
      throw new StoryBibleMutationError("IMMUTABLE_RULE_CHANGE_BLOCKED", "Immutable world rules cannot be changed by this mutation.", 409, { retryable: false });
    }
  }
  if (entityType === "foreshadowing" && field === "status" && previous === "paid" && next !== "paid") {
    throw new StoryBibleMutationError("INVALID_FORESHADOWING_TRANSITION", "Paid foreshadowing cannot be reopened by this mutation.", 409, { retryable: false });
  }
  if (entityType === "open_thread" && field === "status" && previous === "resolved" && next === "open") {
    throw new StoryBibleMutationError("INVALID_OPEN_THREAD_TRANSITION", "Resolved open threads cannot be reopened by this mutation.", 409, { retryable: false });
  }
}
...
async function handleIdempotency(input: { operation: string; candidateId: string; body: JsonRecord; requestId: string; projectId: string; expectedStatus: string; expectedVersion: number }) {
  ...
  if (existing) {
    if (existing.request_hash !== hash && existing.response_hash !== hash) {
      throw new StoryBibleMutationError("IDEMPOTENCY_KEY_REUSED", "The same requestId cannot be reused with a different payload.", 409, { requestId: input.requestId, retryable: false });
    }
...
export async function applyStoryBibleCandidateMutation(candidateId: string, operation: "approve" | "edit-and-approve", body: unknown) {
  ...
  try {
    ...
    if (!candidate) throw new StoryBibleMutationError("CANDIDATE_NOT_FOUND", "Candidate not found in this project.", 404, { traceId: mutationTrace, retryable: false });
    const currentStatus = String(candidate.status || "");
    if (currentStatus !== parsed.expectedCandidateStatus) {
      throw new StoryBibleMutationError("CANDIDATE_STATUS_MISMATCH", "Candidate status changed; reload before reviewing.", 409, {
    ...
    if (!["pending", "needs_review"].includes(currentStatus)) {
      throw new StoryBibleMutationError("CANDIDATE_NOT_REVIEWABLE", "Candidate is not reviewable in its current status.", 409, { traceId: mutationTrace, currentStatus, retryable: false });
    }
    const entityType = EntityTypeSchema.safeParse(candidate.entity_type);
    if (!entityType.success) throw new StoryBibleMutationError("ENTITY_TYPE_NOT_SUPPORTED", "Unsupported Story Bible entity type.", 422, { traceId: mutationTrace, entityType: candidate.entity_type, retryable: false });
    const adapter = adapterFor(entityType.data);
    if (!adapter) throw new StoryBibleMutationError("ENTITY_TYPE_NOT_SUPPORTED", "Unsupported Story Bible entity type.", 422, { traceId: mutationTrace, entityType: candidate.entity_type, retryable: false });
    ...
    if (version.versionNumber !== parsed.expectedStoryBibleVersion) {
      ...
      throw new StoryBibleMutationError("STORY_BIBLE_VERSION_CONFLICT", "Story Bible version changed; reload candidate data before reviewing.", 409, {
        traceId: mutationTrace,
        expectedVersion: parsed.expectedStoryBibleVersion,
    ...
    if (sourceMode === "ai-supported" && (candidate.source_valid === false || sourceRefs.length === 0)) {
      throw new StoryBibleMutationError("INVALID_SOURCE_REFERENCE", "AI-supported approval requires at least one valid source reference.", 409, { traceId: mutationTrace, retryable: false });
    }

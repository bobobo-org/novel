...
export function buildNovelContext(input: ContextBuildInput): BuiltNovelContext {
  ...
  input.characters.forEach((character) => { const text = [character.name, character.identity.value, character.personality.value, character.goal.value, character.portrait?.visualDescription, ...(character.portrait?.traits ?? []), ...(character.rpgProfile ? Object.entries(character.rpgProfile.stats).map(([key, value]) => `${key}=${value}`) : []), character.dynamicsProfile?.archetypeLabel, character.dynamicsProfile?.socialRole, ...(character.dynamicsProfile?.personalityTraits ?? []), ...(character.dynamicsProfile?.relationshipNeeds ?? []), ...(character.dynamicsProfile?.behavioralTendencies ?? [])].filter(Boolean).join("；"); evidence.push({ layer: "L3-character", id: character.id, title: character.name, content: text, score: 50 + relevance(query, text) * 10 }); });

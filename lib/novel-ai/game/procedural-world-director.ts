...
const DIMENSIONS: Record<RpgMode, EncounterDimensions> = {
  adventure: {
    ...
    goals: [
      { id: "secure-exit", label: "保住退路", text: "眼下必須保住能讓眾人安全離場的退路。" },
      { id: "verify-truth", label: "驗明真相", text: "眼下必須取得足以排除假情報的證據。" },
      { id: "protect-witness", label: "護住證人", text: "眼下必須讓關鍵證人能自行說出完整經過。" },
      { id: "reach-target", label: "逼近目標", text: "眼下必須在不失去後援的前提下逼近核心目標。" },
    ],
    pressures: [
    ...
    relationshipTensions: [
      { id: "ally-needs-reason", label: "同伴要求理由", text: "同伴願意協助，卻要求主角先說明真正目的。" },
      { id: "witness-fears-cost", label: "證人畏懼代價", text: "證人知道真相，卻擔心說出口會傷害身邊的人。" },
      { id: "rival-shares-interest", label: "對手暫時同利", text: "對手與主角暫時利益一致，但彼此都不信任。" },
      { id: "leader-tests-loyalty", label: "領隊檢驗承擔", text: "隊伍正在觀察主角是否願意承擔選擇後果。" },
    ],
    costs: [
    ...
    aftermaths: [
      ...
      { id: "clue-fragment", label: "線索碎片", text: "現場只留下能指向下一站、卻還不能拼出真相的碎片。" },
  ...
  cultivation: {
    ...
    goals: [
      { id: "stabilize-state", label: "穩住狀態", text: "眼下必須先穩住身心，才有餘力繼續修行。" },
      { id: "verify-method", label: "驗證功法", text: "眼下必須辨明功法變化究竟來自既有法則，還是一次誤判。" },
      { id: "repair-trust", label: "修補信任", text: "眼下必須在不越過界線的前提下修補合作信任。" },
      { id: "complete-trial", label: "完成試煉", text: "眼下必須以現有境界完成試煉，而不是倚靠突兀的跳級突破。" },
    ],
    pressures: [
    ...
    relationshipTensions: [
      { id: "truth-or-distance", label: "坦白或疏離", text: "坦白限制可能換得理解，也可能拉開距離。" },
      { id: "mentor-withholds", label: "師者保留", text: "指導者願意糾錯，卻拒絕替主角作決定。" },
      { id: "rival-respect", label: "競爭中的尊重", text: "對手承認能力，卻會利用任何重複與鬆懈。" },
      { id: "companion-boundary", label: "同修界線", text: "同行者願意共修，但要求每一步都能撤回同意。" },
    ],
    costs: [
      ...
      { id: "owe-answer", label: "欠下答覆", text: "代價是承諾之後要正面回答一項未解問題。" },
    ],
    deadlines: [
      ...
      { id: "promise-due", label: "承諾到期", text: "眼前這次交涉告一段落前，約定的答覆必須交付。" },
    ],
    reversals: [
    ...
    aftermaths: [
      ...
      { id: "trust-question", label: "信任待答", text: "同行者會記住一個尚未獲得回答的疑問。" },
  ...
  management: {
    ...
    goals: [
      { id: "preserve-runway", label: "守住現金線", text: "眼下必須守住足以維持營運的現金安全線。" },
      { id: "protect-quality", label: "保住品質", text: "眼下必須在交付壓力下保住可驗證的品質底線。" },
      { id: "retain-team", label: "穩住團隊", text: "眼下必須重新分工，避免關鍵人力流失。" },
      { id: "capture-window", label: "抓住窗口", text: "眼下必須以有限投入抓住即將關閉的市場窗口。" },
    ],
    pressures: [
    ...
    relationshipTensions: [
      { id: "team-disagrees", label: "團隊歧見", text: "團隊對速度與品質的優先順序沒有共識。" },
      { id: "client-tests-trust", label: "客戶試探信任", text: "客戶願意等待，卻要求看見可驗證承諾。" },
      { id: "supplier-bargains", label: "供應商談判", text: "供應商能提供協助，但要求更長期的交換。" },
      { id: "leader-owns-result", label: "領導承擔", text: "團隊正在判斷負責人是否會承擔失敗結果。" },
    ],
    costs: [
    ...
    aftermaths: [
      { id: "cash-locked", label: "資金鎖定", text: "投入的資金會有一段時間無法轉作其他用途。" },
      { id: "staff-recovery", label: "人力恢復", text: "參與執行的團隊需要恢復，接下來能調動的人手會受到限制。" },
      ...
      { id: "competitor-response", label: "競爭回應", text: "競爭者會依這次策略調整下一步市場動作。" },
...
export function buildProceduralCausalFrame(input: {
  ...
  const baseDimensions: ProceduralInferenceDimensions = {
    ...
    goal: encounter.goal ?? "眼下必須完成一項可驗證、但有限的目標。",
    ...
    deadline: encounter.deadline ?? "眼前局勢再次改變前必須作出取捨。",
  ...
  return {
    ...
    consequenceBeat: `${reversal}${aftermath}${progressBeat}${recoveryBeat}${closureBeat}這些後果會跟著選擇延續下去；人物不會忘記承諾，失去的時間與資源也不會憑空復原。`,
    hopeGuard: {
      progressKind,
      progressBeat,
      recoveryBeat,

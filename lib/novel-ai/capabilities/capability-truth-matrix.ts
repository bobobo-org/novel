export const CAPABILITY_TRUTH_MATRIX_VERSION = "closed-agent-os-capability-truth-v5" as const;
...
export const CAPABILITY_TRUTH_MATRIX: CapabilityTruthRecord[] = [
  ...
  { id: "browser.aiRuntime", status: "available", evidence: ["WebLLM Web Worker generation with streaming and cancellation", "Transformers.js semantic retrieval", "Chrome built-in capability probe", "packaged extractive model with immutable digest and fixed-input inference proof"], limitations: ["WebGPU generation remains device- and model-install-dependent; the packaged fallback is an extractive light-task model, not a generative LLM"] },
  ...
  { id: "worldWorkbench", status: "verified", evidence: ["project world and world-rule editor", "IndexedDB world repositories", "closed-AI world context composer"], limitations: ["Runtime persistence depends on browser IndexedDB availability"] },
  { id: "characterWorkbench", status: "verified", evidence: ["character editor and private-secret boundary", "directed relationship graph", "approved RPG and visual context integration"], limitations: ["Runtime persistence depends on browser IndexedDB availability"] },
  { id: "rpgMode", status: "verified", evidence: ["governed RPG state and formula suite", "three distinct choices plus free action", "atomic approved story-state effects", "inventory, equipment, currency, quest and relationship tests"], limitations: ["Generative prose quality still depends on the selected and available AI runtime"] },
  { id: "cultivationMode", status: "verified", evidence: ["cultivation progression profile", "procedural encounter and pill suites", "realm resources, spirit stones and non-repeating consequences"], limitations: ["Generative prose quality still depends on the selected and available AI runtime"] },
  { id: "managementMode", status: "verified", evidence: ["management economy formulas", "turn settlement and bounded mutation tests", "cash, revenue, cost, inventory, morale, quality, reputation and risk state"], limitations: ["Generative prose quality still depends on the selected and available AI runtime"] },
  { id: "visualCharacterBible", status: "verified", evidence: ["10,000-entry stable portrait catalog backed by 108 distinct atlas crops with deterministic era-compatible distribution", "local PNG/JPEG/WebP upload pipeline", "approved visual traits in character AI and RPG context"], limitations: ["The 10,000 stable choices derive colour treatments from 108 bundled base identities; they are not 10,000 separately painted faces or a live image-generation service"] },
  ...
  { id: "externalAI", status: "implemented", evidence: ["OpenAI Responses, Gemini Generate Content, xAI Responses and Claude Messages server adapters", "live credential-and-model metadata verification", "same-origin, body, request, token and concurrency guards", "closed-only, hybrid and external-only execution modes", "single-use explicit consent and candidate-only boundary"], limitations: ["Each provider remains unavailable until its server-side credential and model access pass the live probe", "Never selected in closed-only mode", "No silent fallback between external and closed providers", "Distributed account-level quotas still belong to each provider or deployment firewall"] },

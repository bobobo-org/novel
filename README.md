...
- 已合併功能：Explore、快速創作、互動故事、ChatGPT 接力、小型閉端 AI、章節草稿生成、草稿迭代、長程一致性守護、版本分支、多章批量操作、完整作品存檔槽

## 寫作存檔與續寫

- 「本機存檔」會把目前作品存到瀏覽器 `localStorage`，同一台電腦、同一個瀏覽器下次打開會自動讀回。
- 「完整保存到作品槽」會保存完整作品快照，包含設定、故事種子、大綱、章節與記憶，可在「存檔 / 匯出」按「載入續寫」叫回來。
- 「我的書架」也可以保存目前作品並載入續寫。
- 「下載 JSON 備份」適合跨電腦或換瀏覽器保存；之後用「貼上 JSON 匯入」恢復。
- 多章批量操作會先產生候選批次，不直接覆蓋主線；套用前會存版本，套用後會自動保存到作品槽。
